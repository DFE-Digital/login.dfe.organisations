...
const putSingleServiceIdentifierForUser = async (req, res) => {
  ...
  const identifierValue = req.body.id_value
    ? req.body.id_value.toLowerCase()
    : "";
  ...
  if (identifierValue && identifierValue.length > 0) {
    const checkExternalIdentifier = await getExternalIdentifier(
      serviceId,
      identifierKey,
      identifierValue,
    ...
    if (
      checkExternalIdentifier &&
      checkExternalIdentifier.userId.toLowerCase() !== userId
    ) {
