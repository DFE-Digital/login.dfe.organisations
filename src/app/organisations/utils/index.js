...
const getUserOrganisationIdentifiers = async (
  userId,
  organisationId,
  definedNumericIdentifier,
  definedTextIdentifier,
) => {
  let numericIdentifier = definedNumericIdentifier;
  let textIdentifier = definedTextIdentifier;
  const userOrgs =
    (!numericIdentifier || !textIdentifier
      ? await getOrganisationsAssociatedToUser(userId)
      : []) || [];
  const existingUserOrg = userOrgs.find(
    (uo) => uo.organisation.id.toLowerCase() === organisationId.toLowerCase(),
  );
  ...
  if (
    !numericIdentifier &&
    config.toggles &&
    config.toggles.generateUserOrgIdentifiers
  ) {
  ...
  if (
    ...
    while ((!current || inUse) && index <= 5) {
      ...
      inUse =
        exiting &&
        !(
          exiting.user_id === userId &&
          exiting.organisation_id === organisationId
        );
      index += 1;
    }
    if (inUse) {
      throw new Error(
        `No textIdentifier options for numeric identifier ${numericIdentifier} are unused`,
      );
    }
