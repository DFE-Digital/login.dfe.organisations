const { getOrgByUrn, getOrgByUid, getOrgByEstablishmentNumber, getOrgByLegacyId, getOrgByUpin, getOrgByLegalName, getOrgByUkprn, getAllOrgsByUkprn } = require('./data/organisationsStorage');
...
const getOrganisationByExternalIdentifier = async (req, res) => {
  ...
  } else if (req.params.type === 'LegalName') {
    result = await getOrgByLegalName(req.params.id);
