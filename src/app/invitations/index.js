...
const invitationRoutes = () => {
  ...
  router.get(
    "/:inv_id",
    deprecate("/invitations/v2/:inv_id"),
    asyncWrapper(getInvitation),
  );
  router.get("/v2/:inv_id", asyncWrapper(getInvitationV2));
  router.post(
    "/:inv_id/migrate-to-user",
    asyncWrapper(postMigrateInvitationToUser),
  );
...
const organisationRoutes = () => {
  ...
  router.delete(
    ...
    asyncWrapper(deleteInvitationOrganisation),
  );
  router.put(
    "/:org_id/services/:svc_id/invitations/:inv_id",
    asyncWrapper(putInvitation),
  );
