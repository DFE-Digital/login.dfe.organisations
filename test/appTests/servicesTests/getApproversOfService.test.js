...
jest.mock("./../../../src/app/services/data/servicesStorage", () => {
  ...
  return {
    ...
    getApproversOfServiceUserIds: jest
      .fn()
      .mockImplementation(getApproversOfServiceUserIds),
  };
});
...
describe("when getting users of services", () => {
  ...
  it("then the correct params are passed to the storage provider", async () => {
    ...
    expect(servicesStorage.getApproversOfServiceUserIds.mock.calls[0][0]).toBe(
      "1d672383-cf21-49b4-86d2-7cea955ad422",
    );
    expect(servicesStorage.getApproversOfServiceUserIds.mock.calls[0][1]).toBe(
      "9d672383-cf21-49b4-86d2-7cea955ad422",
    );
    expect(servicesStorage.getApproversOfServiceUserIds.mock.calls[0][2]).toBe(
      expectedRequestCorrelationId,
    );
  });
});
