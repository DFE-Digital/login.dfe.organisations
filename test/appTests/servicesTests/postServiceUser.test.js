...
describe("when posting to link a user to a service", () => {
  ...
  it("then if the request is valid it is passed to the create command", async () => {
    ...
    expect(servicesStorage.upsertServiceUser.mock.calls[0][0].userId).toBe(
      req.params.uid,
    );
    expect(
      servicesStorage.upsertServiceUser.mock.calls[0][0].organisationId,
    ).toBe("org1");
    expect(servicesStorage.upsertServiceUser.mock.calls[0][0].serviceId).toBe(
      req.params.sid,
    );
